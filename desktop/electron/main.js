...
function startBackend() {
  ...
  backendProcess.on("error", (err) => {
    // Sans ce handler, une erreur de spawn (exe introuvable, mis en
    // quarantaine par l'antivirus, permissions...) est un événement
    // 'error' non écouté sur un EventEmitter Node -> exception non
    // rattrapée qui fait planter TOUT le processus Electron (crash natif
    // Windows "l'application a cessé de fonctionner"), au lieu d'un
    // message propre à l'utilisateur.
    dialog.showErrorBox(
      "CrimTrack — impossible de démarrer le service",
      "Le programme backend n'a pas pu être lancé (" + err.message + "). " +
        "Vérifiez que crimtrack-backend.exe n'a pas été mis en quarantaine par " +
        "votre antivirus, et ajoutez une exception si nécessaire."
    );
    app.quit();
  });
