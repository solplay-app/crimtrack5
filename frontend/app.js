...
// ---------------------------------------------------------------------------
// Filet de rattrapage — jusqu'ici aucun écran de l'app n'était protégé
// contre une exception pendant le rendu (ex. calcul du graphe D3 sur un
// jeu de données très fourni) : sans ErrorBoundary, React démonte tout
// l'arbre et l'utilisateur se retrouve face à un écran vide/figé, sans
// message ni moyen de s'en sortir autrement qu'en rechargeant la page à la
// main. On isole chaque écran routé pour que ce soit lui, pas toute
// l'app, qui tombe — avec un vrai message et un bouton de reprise.
// ---------------------------------------------------------------------------
class ErrorBoundary extends React.Component {
  ...
  componentDidCatch(error, info) {
    console.error("CrimTrack — erreur d'affichage :", error, info?.componentStack);
  }
  render() {
    if (this.state.error) {
      return html`
        <div class="panel" style=${{ margin: "20px" }}>
          <p class="panel-title">Cet écran a rencontré un problème d'affichage</p>
          <p class="page-sub">${this.state.error.message || String(this.state.error)}</p>
          <button class="btn" onClick=${() => this.setState({ error: null })}>Réessayer</button>
        </div>
      `;
    }
...
function RelationGraph({ nodes, edges, height, highlightIds }) {
  ...
  useEffect(() => {
    ...
    // Au-delà d'un certain nombre de liens, une étiquette de texte par lien
    // (remise à jour à chaque tick de la simulation, ~300 ticks) devient le
    // principal poste de coût CPU et peut geler l'onglet plusieurs secondes
    // sur un graphe un peu fourni (ex. beaucoup d'incidents partageant une
    // adresse ou plusieurs véhicules/personnes). On les masque au-delà du
    // seuil plutôt que de risquer un gel — un survol via <title> reste
    // disponible sur chaque nœud.
    const LABEL_THRESHOLD = 120;
    const showLinkLabels = simEdges.length <= LABEL_THRESHOLD;
    ...
    const linkLabel = showLinkLabels
      ? g.selectAll("text.link-label").data(simEdges).join("text")
...
function RelationsPage() {
  ...
  const cheminHighlightIds = useMemo(
    () => (cheminResult && cheminResult.trouve ? cheminResult.nodes.map((n) => n.id) : []),
    [cheminResult]
  );
            ...
            ${!cheminResult.trouve ? html`<${Empty} label="Aucun chemin trouvé entre ces deux entités." />` : html`
              ...
              <div style=${{ marginBottom: "12px" }}>
                <${RelationGraph}
                  ...
                  height=${260}
                  highlightIds=${cheminHighlightIds}
                />
              </div>
              <div>
...
function AppShell() {
  ...
  return html`
    <div class="shell">
      ...
      <div class="main"><${ErrorBoundary} key=${hash}>${content}<//></div>
    </div>
  `;
}
